...
ExpressionName:	 Identifier DOT Identifier				{$$=new Typeinfo;string x=$1;string p=newtemp(); $$->tempname=p;p=p+" = "+x; $$->tac={p};}	
		...
		;
MethodName:	 UnqualifiedMethodIdentifier				{$$=new Typeinfo; $$->tac=$1->tac;}	
		;
...
ClassMemberDeclaration:	 FieldDeclaration		{$$=new Typeinfo; $$->tac=$1->tac;}							
		...
		| SEMICOLON								{$$=new Typeinfo; $$->tac=emptyvector();}						
		;
FieldDeclaration:	 UnannType VariableDeclaratorList SEMICOLON     	{$$=new Typeinfo; $$->tac=$2->tac;}	
		| OMClassModifier UnannType VariableDeclaratorList SEMICOLON	{$$=new Typeinfo; $$->tac=$3->tac;}	
		;
VariableDeclaratorList:	 VariableDeclarator				{$$=new Typeinfo;$$->tac=$1->tac;}				
		| VariableDeclarator COMMA VariableDeclaratorList		{$$=new Typeinfo;$$->tac=concvector($1->tac,$3->tac);}
		;
VariableDeclarator:	 VariableDeclaratorId						{$$=new Typeinfo; $$->tac=emptyvector();}	
		| VariableDeclaratorId EQUAL VariableInitializer		{$$=new Typeinfo;string c=$1->tempname+"="+$3->tempname;$$->tac=concvector($1->tac,$3->tac,c);}
		;
VariableDeclaratorId:  Identifier							{$$=new Typeinfo;string x=$1;$$->tempname=x;}			
		|	 Identifier OMPSB								{$$=new Typeinfo;string x=$1;$$->tempname=x;}						
		;
...
MethodDeclaration:	 MethodHeader MethodBody					{$$=new Typeinfo;string p=newlabel(); string z="function end\n"+p; $$->tac=concvector($1->tac,$2->tac,z);}		
		| OMClassModifier MethodHeader MethodBody				{$$=new Typeinfo;string p=newlabel(); string z="function end\n"+p; $$->tac=concvector($2->tac,$3->tac,z);}	
		;
MethodHeader:	 Result MethodDeclarator		                {$$=new Typeinfo;string q=newlabel();string p="call "+($1)->tempname+"\n"+q;$$->tac=concvector(p,$2->tac);}	
		| Result MethodDeclarator Throws						{$$=new Typeinfo;string q=newlabel();string p="call "+($1)->tempname+"\n"+q; $$->tac=concvector(p,$2->tac);}		
		| TypeParameters Result MethodDeclarator				{$$=new Typeinfo;string q=newlabel();string p="call "+($2)->tempname+"\n"+q; $$->tac=concvector(p,$3->tac);}		
		| TypeParameters Result MethodDeclarator Throws			{$$=new Typeinfo;string q=newlabel();string p="call "+($2)->tempname+"\n"+q; $$->tac=concvector(p,$3->tac);}		
		;
...
StaticInitializer:	 STATIC Block				{$$=new Typeinfo; $$->tac=$2->tac;}							
		;	
ConstructorDeclaration:	 ConstructorDeclarator ConstructorBody			{$$=new Typeinfo;string z="constructor called";string zz="constructor end"; $$->tac=concvector(z,$1->tac,$2->tac,zz);}
		| ConstructorDeclarator Throws ConstructorBody				    {$$=new Typeinfo;string z="constructor called";string zz="constructor end"; $$->tac=concvector(z,$1->tac,$2->tac,zz);}
		| OMClassModifier ConstructorDeclarator ConstructorBody			    {$$=new Typeinfo;string z="constructor called";string zz="constructor end"; $$->tac=concvector(z,$1->tac,$2->tac,zz);}
		| OMClassModifier ConstructorDeclarator Throws ConstructorBody	{$$=new Typeinfo;string z="constructor called";string zz="constructor end"; $$->tac=concvector(z,$1->tac,$2->tac,zz);}
		;
ConstructorDeclarator:   Identifier LRB RRB									{$$=new Typeinfo; $$->tac=emptyvector();}				
		|	 Identifier LRB FormalParameterList RRB								{$$=new Typeinfo;$$->tac=$3->tac;}					
		...
		|	 TypeParameters Identifier LRB RRB								{$$=new Typeinfo; $$->tac=emptyvector();}				
		|	 TypeParameters Identifier LRB FormalParameterList RRB						{$$=new Typeinfo;$$->tac=$4->tac;}			
...
InterfaceMemberDeclaration:	 ConstantDeclaration					{$$=new Typeinfo;$$->tac=$1->tac;}	
		...
		| SEMICOLON															{$$=new Typeinfo;$$->tac=emptyvector();}
		;
ConstantDeclaration:	 UnannType VariableDeclaratorList SEMICOLON			{$$=new Typeinfo;$$->tac=$2->tac;}
		| OMClassModifier UnannType VariableDeclaratorList SEMICOLON		{$$=new Typeinfo;$$->tac=$3->tac;}
		;
...
LocalVariableDeclarationStatement:	 LocalVariableDeclaration SEMICOLON		{$$=new Typeinfo; $$->tac= $1->tac;}
		;
LocalVariableDeclaration:	 LocalVariableType VariableDeclaratorList		{$$=new Typeinfo; $$->tac= $2->tac;}
		| OMClassModifier LocalVariableType VariableDeclaratorList			{$$=new Typeinfo; $$->tac= $3->tac;}
		;
...
ArrayAccess:   Identifier LSB Expression RSB			{$$=new Typeinfo; $$->tac=$3->tac;}
		...
		;
MethodInvocation:   MethodName LRB ArgumentList RRB				{$$=new Typeinfo; $$->tac=concvector($1->tac,$3->tac);}				
		|	 MethodName LRB RRB									{$$=new Typeinfo; $$->tac=$1->tac;}			
